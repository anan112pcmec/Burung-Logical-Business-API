...
func DataAlamatEkspedisiUp(db *gorm.DB) {
	...
	if err := db.Model(&sot_models.AlamatEkspedisi{}).Pluck("id", &idsAlamatEks).Error; err != nil {
		return
	}

	var alamatEkspedisiData []sot_models.AlamatEkspedisi
	if err := db.Where("id IN ?", idsAlamatEks).Find(&alamatEkspedisiData).Error; err != nil {
		return
	}

	if data_cache.DataAlamatEkspedisi == nil {
		data_cache.DataAlamatEkspedisi = make(map[string]map[int64]sot_models.AlamatEkspedisi, len(alamatEkspedisiData))
	}

	for i := range alamatEkspedisiData {
		kota := alamatEkspedisiData[i].Kota
		if _, ok := data_cache.DataAlamatEkspedisi[kota]; !ok {
			data_cache.DataAlamatEkspedisi[kota] = make(map[int64]sot_models.AlamatEkspedisi)
		}
		data_cache.DataAlamatEkspedisi[kota][alamatEkspedisiData[i].ID] = alamatEkspedisiData[i]
	}
}

func DataAlamatEkspedisiChange(DataEkspedisi sot_models.AlamatEkspedisi, kota string, id int64, db *gorm.DB) {
